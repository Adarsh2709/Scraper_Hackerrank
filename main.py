...
class HackerrankLeaderboard:
    ...
    def create_input_field(self):
        ...
        self.entry.place(x=20, y=120, width=1101, height=391)
        self.entry["insertbackground"] = "#FFE33E"

    def create_cookie_field(self):
        # Cookie Label
        self.cookie_label = tk.Label(
            self.root,
            text="Session Cookie (Optional - for private contests):",
            font=tkFont.Font(family='Helvetica', size=12, weight='bold'),
            fg="#FFE33E",
            bg='#404445',
            anchor="w"
        )
        self.cookie_label.place(x=20, y=515, width=400, height=20)
        ...
        self.cookie_entry = tk.Entry(
            ...
            background="black",
            fg="#FFE33E",
            insertbackground="#FFE33E",
            font=tkFont.Font(family='Times', size=16),
            borderwidth="3px",
            relief="groove"
        )
        self.cookie_entry.place(x=20, y=535, width=1101, height=30)
    ...
    def extract_contest_id(self, input_text):
        """Extract contest ID from a URL or return the text itself."""
        # Check if multiple IDs are provided (comma separated)
        ids = [id.strip() for id in input_text.split(',') if id.strip()]
        processed_ids = []
        
        for item in ids:
            if 'hackerrank.com/' in item:
                match = re.search(r'contests/([^/]+)', item)
                if match:
                    processed_ids.append(match.group(1))
                else:
    ...
    def fetch_hackerrank_data(self, tracker_name, progress_window, progress_text):
        data = []
        ...
        while True:
            url = f'https://www.hackerrank.com/rest/contests/{tracker_name}/leaderboard?offset={offset}&limit={limit}'
            try:
                response = requests.get(url, headers=headers, timeout=15)
                
                if response.status_code == 403:
                    self.update_progress(progress_window, progress_text, None, 
                                        f"Error 403: Access forbidden for {tracker_name}. Contest might be private.\n", 0)
                    return None
                ...
                offset += limit
                self.update_progress(progress_window, progress_text, None, 
                                    f"Fetched {len(data)}/{total} for {tracker_name}...\n", 0)

            except requests.RequestException as e:
                self.update_progress(progress_window, progress_text, None, 
                                    f"Failed to fetch data for {tracker_name}: {str(e)}\n", 0)
                return None
    ...
    def generate_sheets_thread(self, tracker_names, progress_window, progress_text, progress):
        try:
            ...
            for idx, tracker_name in enumerate(tracker_names, 1):
                df = self.fetch_hackerrank_data(tracker_name, progress_window, progress_text)
                if df is None:
                    continue
    ...
    def generate_sheets_command(self):
        ...
        try:
            contest_ids = self.extract_contest_id(inp)
            if not contest_ids:
